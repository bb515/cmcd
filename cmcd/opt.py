...
def create_optimizer(step_size, b1=0.9, b2=0.999, eps=1e-8):
    clip = optax.clip(5.0)
...
def run(
  ...
  losses = []
  looper = tqdm(range(iters)) if config.training.run_cluster == 0 else range(iters)

  for i in looper:
    # TODO: BB depends on what grad and loss is as to what i is a loop over
    # is it iterations over the full dataset?
    rng_key, rng_key_gen = jax.random.split(rng_key_gen)
    seeds = jax.random.randint(rng_key, (config.training.batch_size,), 1, 1e6)
