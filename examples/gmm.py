...
from diffusionjax.plot import plot_heatmap, plot_scatter
...
from cmcd.utils import get_sampler as get_cmcd_sampler
from cmcd.solvers import AISUDLangevin
...
def generate_measurement_equations(dim, dim_y, rng, mixt, noise_std):
  A = random.normal(rng, shape=(dim_y, dim))
  rng, step_rng = random.split(rng, 2)
  u, diag, coordinate_mask, v = build_extended_svd(A)
  diag = jnp.flip(jnp.sort(random.uniform(step_rng, shape=init_obs.shape)))
  A = u @ (jnp.diag(diag) @ v[coordinate_mask==1, :])
  init_sample = mixt.sample()
  init_obs = A @ init_sample
  init_obs = init_obs + random.normal(step_rng, shape=init_obs.shape) * noise_std
  Sigma_y = jnp.diag(noise_std**2 * jnp.ones(len(diag)))
  posterior = get_posterior(init_obs, mixt, A, Sigma_y)
  return A, Sigma_y, u, diag, coordinate_mask, v, posterior, init_obs
...
def main(argv):
  ...
  for ind_dim, dim in enumerate([8, 80, 800]):
    ...
    means = []
    for i in range(-2, 3):
      means += [jnp.array([-size * i, -size * j] * (config.data.image_size//2)) for j in range(-2, 3)]
    ...
    plot_scatter(target_samples, index=(0, 1), fname="target gmm jax")

    logging.info("target prior:\nmean {},\nvar {}".format(np.mean(target_samples, axis=0), np.var(target_samples, axis=0)))
    ...
    outer_solver = AISUDLangevin(score, num_steps=config.solver.num_outer_steps,
    ...
    sampler = get_cmcd_sampler(sampling_shape, outer_solver,
